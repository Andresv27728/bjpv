...
user.coin -= count * 1
if (Aku > Kamu) {
conn.reply(m.chat, '`🍭 Veamos que numeros tienen!`\n\n'+ `➠ *${botname}* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username}, *PERDISTE* ${formatNumber(count)} 🪙 ${moneda}.`.trim(), m, rcanal)
} else if (Aku < Kamu) {
user.coin += count * 2
conn.reply(m.chat, '`🍭 Veamos que numeros tienen!`\n\n'+ `➠ *${botname}* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username}, *GANASTE* ${formatNumber(count * 2)} 🪙 ${moneda}.`.trim(), m, rcanal)
} else {
user.coin += count * 1
conn.reply(m.chat, '`🍭 Veamos que numeros tienen!`\n\n'+ `➠ *${botname}* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username} obtienes ${formatNumber(count * 1)} 🪙 ${moneda}.`.trim(), m, rcanal)}
